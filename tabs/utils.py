...
def get_all_from_api_query(
    ...
    while get_link_next_page(r.json(), next_page):
        while True:
            try:
                r = requests.get(
                    get_link_next_page(r.json(), next_page), headers=headers, timeout=5
                )
                break
            except Exception as e:
                print(e)
...
DATASERVICES_QUALITY_METRICS = [
    {"label": "URL de l'API", "value": "base_api_url"},
    {"label": "Point de contact", "value": "contact_point"},
    {"label": "Description sur data.gouv", "value": "description"},
    {"label": "URL de description", "value": "endpoint_description_url"},
